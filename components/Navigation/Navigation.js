...
const Navbar = ({ toggle, route, router, ...props }) => {
  return (
    ...
    >
      <Section>
        <div className="flex  md:py-0 space-x-10 items-center justify-center">
        ...
        <div className="flex text-[16px] xl:text-[20px]">
          ...
          <Link
            href="/about"
            passHref
            className={`p-4 hidden md:flex items-center gap-2`}
          >
            <div className={props.className}>ABOUT US</div>
          </Link>
          <Link
            href="/work"
            passHref
            className={`p-4 items-center gap-2 hidden md:flex`}
          >
            <div className={props.className}>OUR WORKS</div>
          </Link>
          <Link
            href="/contact"
            passHref
...
const Sidebar = ({ isOpen, toggle, route, ...props }) => {
  return (
    <div
      className={
        isOpen
          ? "fixed z-30 bg-[#141414] top-0 w-full h-full lg:hidden translate-y-0 duration-700 transform ease-out"
          : "translate-y-[-100%]"
      }
    >
      <div
      ...
      >
        <div className="flex py-2 items-center w-full justify-between px-4 sm:px-0 z-10 max-w-[500px]">
          <div className="flex items-center relative w-[125px] h-[45px] md:w-[170px] md:h-[65px] xl:w-[224px] xl:h-[82px]">
            <Link href="/" passHref className="font-extrabold text-[24px]">
              <div className="relative w-[42px] aspect-[1.3857]">
                <Image
                  src="/icons/logo.svg"
                  alt="logo"
                  fill
        ...
        <div className="w-full flex flex-col h-full justify-center items-center z-40">
          ...
          >
            <li className="w-full">
              <Link href="/" passHref className={`block px-5 text-[22px]`}>
                <div>HOME</div>
              </Link>
            </li>
            <li className="w-full">
